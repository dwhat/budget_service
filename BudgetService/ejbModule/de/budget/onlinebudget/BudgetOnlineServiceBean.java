...
//TODO �berall Session auf null pr�fen um nullpointer zu vermeiden, siehe getItemByLossCategory
...
public class BudgetOnlineServiceBean implements BudgetOnlineService {
	...
	/**
	 * Session anhand username und password erstellen und in ResponseObject zur�ckliefern
	 * 
	 * @author Moritz
	 * @param Username 
	 * @param password
	...
	public UserLoginResponse login(String username, String password) {
		...
		try 
		{
			...
			//evtl darauf einigen einen 404(Not-Found) zur�ckzuschicken 
		}
		return response;
	}

	/**
	 * UserSession zerst�ren
	 * 
	 * @author Moritz
	 * 
	 */
	...
	public UserLoginResponse setUser(String username, String password, String email) {
		...
		try {
			...
			// TODO pr�fen ob Username bereits verwendet wurde
	...
	public ReturnCodeResponse deleteUser(int sessionId, String username) {
		...
		try {
			BudgetSession session = getSession(sessionId);
			if (session != null) {
				if(session.getUsername().equals(username)) { //pr�ft ob man den eigenen user L�scht
					dao.deleteUser(username);
					logger.info("User erfolgreich gel�scht");
				}
				else {
					response.setMessage("Sie k�nnen keinen fremden User l�schen");
				}
			}
		}
		catch(NoSessionException e) {
	...
	public BasketListResponse getBasketsByPayment(int sessionId, int paymentId) {
		...
		try {
			...
			if (session != null) {
				User user = this.dao.findUserByName(session.getUsername());
				List<Basket> basketList = new ArrayList<>(); //List with all baskets of the payment, wird sp�ter bef�llt
				List<Basket> basketsOfUser = user.getBaskets(); //List with all baskets of the user
	...
	public ReturnCodeResponse deleteBasket(int sessionId, int basketId) {
		...
		try {
			...
			if (session != null) {
				...
				logger.info("Basket erfolgreich gel�scht");
			}
		}
		catch(NoSessionException e) {
			response.setReturnCode(500);
	...
	public ReturnCodeResponse deleteVendor(int sessionId, int vendorId) {
		...
		try {
			...
			if (session != null) {
				...
				logger.info("Vendor erfolgreich gel�scht");
			}
		}
		catch(NoSessionException e) {
			response.setReturnCode(500);
	...
	public ReturnCodeResponse deletePayment(int sessionId, int paymentId) {
		...
		try {
			...
			if (session != null) {
				...
				logger.info("Payment erfolgreich gel�scht");
			}
		}
		catch(NoSessionException e) {
			response.setReturnCode(500);
	 ...
	 * @author Moritz
	 * @param sessionId
	 * @param paymentId alte Id zum finden des Update Datensatzes n�tig, bei Neuanlage negativen Wert benutzen
	 * @param name
	 * @param number
	 * @param bic
	 * @param active
	...
	public ReturnCodeResponse deleteCategory(int sessionId, int categoryId) {
		...
		try {
			...
			if (session != null) {
				...
				logger.info("Category erfolgreich gel�scht");
			}
		}
		catch(NoSessionException e) {
			response.setReturnCode(500);
	...
	public IncomeListResponse getIncomesByCategory(int sessionId, int categoryId) {
		...
		try {
			...
			if (session != null) {
				User user = this.dao.findUserByName(session.getUsername());
				List<Income> incomeList = new ArrayList<Income>(); // List with all Incomes with the category. wird sp�ter bef�llt
	...
	public ReturnCodeResponse deleteIncome(int sessionId, int incomeId) {
		...
		try {
			...
			if (session != null) {
				...
				logger.info("Income erfolgreich gel�scht");
			}
		}
		catch(NoSessionException e) {
			response.setReturnCode(500);
	...
	public ItemListResponse getItemsByLossCategory(int sessionId, int categoryId) {
		...
		try {
			...
			if (session != null) {
				User user = this.dao.findUserByName(session.getUsername());
				List<Item> itemList = new ArrayList<Item>(); // List with all Items with the category. wird sp�ter bef�llt
	...
	public ReturnCodeResponse deleteItem(int sessionId, int itemId) {
		...
		try {
			...
			if (session != null) {
				...
				logger.info("Item erfolgreich gel�scht");
			}
		}
		catch(NoSessionException e) {
			response.setReturnCode(500);
